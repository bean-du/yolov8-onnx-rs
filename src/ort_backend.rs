...
// 定义ONNXRuntime执行提供者的枚举类型
#[derive(Debug, Clone, PartialEq, Eq, PartialOrd, Ord)]
pub enum OrtEP {
    Cpu,       // CPU执行提供者
    Cuda(u32), // CUDA执行提供者，u32是设备ID
    Trt(u32),  // TensorRT执行提供者，u32是设备ID
}
...
pub struct OrtInputs {
    pub shapes: Vec<Vec<i32>>,           // 输入的形状
    pub dtypes: Vec<TensorElementDataType>, // 输入的数据类型
    pub names: Vec<String>,             // 输入的名称
    pub sizes: Vec<Vec<u32>>,           // 输入的大小
}
...
pub struct OrtConfig {
    pub f: String,                      // ONNX模型文件路径
    pub task: Option<YOLOTask>,         // YOLO任务类型
    pub ep: OrtEP,                      // 执行提供者类型
    pub trt_fp16: bool,                 // 是否启用TensorRT的FP16模式
    pub batch: Batch,                   // 批处理配置
    pub image_size: (Option<u32>, Option<u32>), // 图像大小（高度，宽度）
}

// 定义ONNXRuntime后端的结构体
#[derive(Debug)]
pub struct OrtBackend {
    session: Session,                   // ONNXRuntime会话
    task: YOLOTask,                     // YOLO任务类型
    ep: OrtEP,                          // 执行提供者类型
    batch: Batch,                       // 批处理配置
    inputs: OrtInputs,                  // 输入配置
}
